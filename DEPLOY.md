...
# backend/db/coupang-links.sql = 재료별 쿠팡 파트너스 딥링크(id 기준 UPDATE). 시드 뒤에 실행
docker compose exec -T mysql sh -c 'exec mysql -uroot -p"$MYSQL_ROOT_PASSWORD" ttukttakbap' < backend/db/coupang-links.sql
```
